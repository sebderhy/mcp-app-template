...
2. **Orthogonal test suite** - 282 tests that pass regardless of what app you build
3. **Local ChatGPT Simulator** - Test widgets locally without deploying to ChatGPT
4. **Working examples** - 8 widget examples to learn from or build upon
...
### Test Locally with the Simulator

The easiest way to test your widgets without connecting to ChatGPT:
...
2. Configure the model in `server/simulator_config.json`:
   ```json
   {
     "model": "gpt-4o-mini",
...
3. Start the server and open the simulator:
   ```bash
   pnpm run server
   # Open http://localhost:8000/assets/simulator.html
   ```

4. Try prompts like *"Show me a carousel of restaurants"* or *"Display a dashboard"*

The simulator renders widgets inline just like the real ChatGPT, with expand-to-fullscreen support.
...
- **Simulator** (`src/simulator/`) - Local development UI using OpenAI Agents SDK
- **Assets** (`assets/`) - Built widget bundles (generated by `pnpm run build`)

## Local Simulator

The simulator lets you test widgets without deploying to ChatGPT:
...
| **OpenAI Agents SDK** | Uses real OpenAI API with MCP protocol |

**Configuration** (`server/simulator_config.json`):
```json
{
  "model": "gpt-4o-mini",
  "mcp_server_url": "http://localhost:8000/mcp",
